			...
			{addHelpData.map((currentHelp) => {
				return (
					<div className="card">
						...
						<div>
						<Link to={`/help-detail/${currentHelp._id}`}><button className="btn btn-secondary btn-sm">Help Details</button></Link>
						</div>			
					</div>
			...
			<div>
				{addPostData.map((currentPost) => {
					return (
						...
						<div>
							<Link to={`/post-detail/${currentPost._id}`}><button class="btn btn-success btn-sm">Post Details</button></Link>
						</div>
					</div>
					);
				})}
